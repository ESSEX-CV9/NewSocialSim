...
/** 与路由路径冲突或具有特殊含义的 handle，禁止注册 */
const RESERVED_HANDLES = new Set(['suggested', 'me', 'admin']);
...
export class AuthService {
  ...
  register(input: RegisterRequest): UserProfile {
    ...
    if (RESERVED_HANDLES.has(input.handle.toLowerCase())) {
      throw new ValidationError(`@${input.handle} 是保留用户名`);
    }
