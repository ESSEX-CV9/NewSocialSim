...
1. **ECS 骨架 + tick 引擎最小闭环**：simulator 工作区搭建 → ECS 基础框架 → 模板版内容生成（规则引擎 + 内容池）→ 一个账号在测试世界按 tick 自动发帖，同步做对应的前端监控界面。
2. **世界管理与编辑**：世界元数据编辑 / 时钟控制 / 快照 / 设定文件库 / 账号设计器的 API 与 UI，含预填内容的管理端 API。
3. **话题系统与互动级联**：话题生命周期管理 + 新帖扇出粉丝反应判定。
4. **LLM Agent 接入**：LLMProvider 抽象 + Agent 工具集 + agentic 循环 → 核心 NPC 的 LLM 驱动发帖 / 回复。
5. **GM 导演层**：GM 调度循环 + 任务分发 → 爆款帖生成 / 话题注入 / 内容池补水。
