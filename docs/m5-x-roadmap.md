...
- **改动**：`simulator/src/`（本地控制端点 + 预览 + 池子文件 fs watch 热重载；控制端口入基础设施配置）、`editor/src/server/`（池读写端点落文件 + 预览代理）、`editor/src/renderer/`（内容池面板：三层浏览 + 增删改 + 预览）。
- **范围**：第一口打通"浏览 + 增删改（**含从零新建池 / 语法 / 组件，先用朴素表单**）+ 预览 + 热生效"主线；引导式新建向导、自定义维度键 UI 紧接着补。
- **验收**：在面板新建或修改一个池并点预览，即时看到该池经模拟器组装出的若干条文本；保存后不重启、模拟器下一次发帖即用新定义；模拟器未运行时预览给优雅提示而非报错。
- **交接提示**：现 `POST/GET/DELETE /api/admin/content-pools` 为扁平 `string[]` 模型、已被三层模型取代且模拟器不再读，本步走编辑器后端直读写世界文件夹的三层池文件（不经 social server admin）。预览须用模拟器真引擎而非编辑器自算副本——为内容生成必然演进到依赖 NPC 活状态 / LLM 预留（"为必然到来的需求设计"的防御性原则）。文件归属对作者透明：改谁存回其来源文件，新建条目落到合理默认文件。
