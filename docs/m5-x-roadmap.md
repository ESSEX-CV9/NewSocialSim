...
- **目标**：定义决策轨迹事件结构，模拟器每次写世界（发帖 / 回复 / 引用 / 赞 / 转 / 关注）追加一条 JSONL 到该世界文件夹，作为可审计的持久真相源。
- **改动**：`shared/src/types/trace.ts`（新增 `SimTraceEvent` 类型并在 `shared/src/index.ts` 导出）；`simulator/src/` 新增轨迹 sink 模块（按当前活动世界写 `data/worlds/<id>/sim-trace.jsonl`，无 BOM、追加、原子）；`PostingSystem` 等写世界处调用 sink 吐事件。
- **验收**：在 `demo` 世界跑模拟器，打开 `data/worlds/demo/sim-trace.jsonl`，每条含 `at` / `simTime` / `entity` / `action` / `shape` / `poolId` 等字段，行数随发帖增长。
- **交接提示**：事件字段以 `docs/m5-x-re-plan.md`「决策轨迹」节为准（`at` 真实时间、`simTime` 模拟时间、`entity` handle、`action`、`activityState`、`intent`、`shape`、`poolId`、`entryId`、`mediaAttached`、`mediaReason`、`targetPostId`）。Step 0 阶段 `intent` / `activityState` 可填占位值（如 `earnest` / `null`），后续状态机阶段补真值。sink 写盘路径随活动世界变化，切世界时切文件句柄。
...
- **目标**：编辑器后端读取活动世界的 `sim-trace.jsonl`，提供按模拟时间区间查询接口与一条 SSE 推流端点（先空推）。
- **改动**：`editor/src/server/`（读 per-world JSONL + 区间索引 + `GET /trace?from&to` + `GET /trace/stream` SSE）。
- **验收**：curl 编辑器后端 `GET /trace`，拿到 `demo` 世界已落盘的轨迹事件数组，区间参数生效。
- **交接提示**：JSONL 是真相源，编辑器后端只读不写。后端与世界文件夹同机，直接读文件即可，不经社交站 server 中转。SSE 端点须自管连接（参考社交站 `core/events/SseHub` 的连接范式）。
...
- **LLM 行为层接入**：紧密长帖与核心 NPC 现场生成、Agent 工具范式与 agentic 循环、盲评、提示词缓存纪律、预算降级。
- **GM 导演层（M5-5）**：GM 唤醒控制器（真人活动 / 预设事件 / 内容池水位低 / 兜底间隔四触发）、任务分发、决策日志、预算降级、内容池补水。
