...
interface PoolEntry {
  ...
  intent?: 'attack'|'mock'|'support'|'lament'|'flex'|'flat'|'earnest'
         | 'meta-snark'|'self-deprecating'|'bragging'|'rebuttal'|'praise';

  speakerFaction?: string[];     // 此条仅当说话人 factions 命中其中之一时可选（空 = 任意）
  targetFaction?: string[];      // 此条仅当目标对象 factions 命中其中之一时可选（空 = 任意）

  preferredAlignment?: {         // 软权重 bias，非过滤
    lawfulness?: number;
    morality?: number;
    tolerance?: number;          // 距离容差
  };

  shape: 'standalone' | 'reply' | 'quote' | 'interjection';
  composable?: { asPrefix?: boolean; asSuffix?: boolean };

  topics?: string[];             // 仅在这些话题语境下触发（空 = 任意）
...
NPC 一旦 factions 命中条目的 `speakerFaction`，即有资格抽到。`poolAffinities` 提供细粒度偏好——同一二游玩家可"不爱掺和舟原战但爱聊 FGO"。

派系（faction）是世界级注册表：每个世界维护自己的 faction 列表，新建 NPC 时从列表选。跨世界的派系池靠映射链接（"原神玩家"在 A 世界对应 X faction id、在 B 世界对应 Y，映射到同一基础池）。

### 占位符

引擎在解析池条目文本时扫描 `{key}` 与 `{key:variant}` 形态的占位符，按下表填充：

| 占位符 | 来源 |
|---|---|
| `{slang}` | universal-slang 池中 shape=interjection 的条目随机抽 |
...
| `{pos_word}` / `{neg_word}` | 通用褒贬词池 |

faction 的多 variant 别名存于派系注册表，每个 faction 可登记多种称呼。

占位符填不到匹配项时，引擎丢弃该 PoolEntry 候选重抽，不输出病句。

### 选择算法

NPC 触发"发帖/回复"事件，意图为 `intent`（由 activity + mood 决定）：

1. **候选池** = 基础原子池 ∪ NPC.factions 命中的场景池 ∪ 当前 active 话题对应的话题池
2. **过滤条目**：
   - `intent` 匹配
   - `shape` 匹配当前行动
   - `speakerFaction` 包含 NPC 的某个 faction（或为空）
   - `targetFaction` 兼容（Beefing 时 target faction 必须对上）
   - `topics` 兼容
   - 占位符可解析
3. **加权**：
   ```
   weight = poolAffinity[entry.pool]
          × alignmentMatchFactor(entry, NPC)         // 1.5 / 1.0 / 0.6
          × noveltyFactor(entry, NPC.memory)         // 最近用过的打折
          × topicRelevance(entry, currentTopic)
   ```
4. **抽样** main entry
5. **组合修饰**：`rand() < persona.slangDensity` 时再从 composable=asPrefix 的池抽一条加前缀；asSuffix 同理（独立判定）
6. **占位符解析**
...
候选池：universal-slang、二游池、舟原战池。

候选条目（经过滤）：
- `"急了急了，{target_faction:贬}典中典"`（舟原战）
- `"{target_faction}玩家又开始绷不住了"`（舟原战）
- `"乐"`（universal-slang, standalone+reply）
...
- 内部 EventBus
- PoolEntry schema 扩 `intent` / `composable` / `preferredAlignment` 字段
