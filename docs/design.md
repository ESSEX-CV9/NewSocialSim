...
账号的"类型"即其**默认驱动模式**——一个运行时基线而非固定身份。除真人账号外，GM 均可视情况临时介入。五个命名预设：

| 预设 | 内容来源 | 互动来源 | 内态 | LLM 成本 |
|---|---|---|---|---|
| 真人账号 | 真人 | 真人 | — | — |
| 纯 LLM | LLM 单调 | LLM | 持久 | 最高 |
| 半 LLM | LLM 单调 | 引擎 | 持久 | 高 |
| 纯模拟器 | 池组装 | 引擎 | 持久 | 零 |
| 一次性氛围 | 池组装 / 批量 | 极少 / 无 | 无 | 池化均摊 |

真人账号是唯一铁墙，GM 与模拟器永不触碰。纯 LLM 用于极重要核心 NPC——用户在意其任何 OOC（含点错赞），故其互动也由 LLM 把关、退出确定性互动层。每个被驱动账号另有降级地板：`degrade` 在资源不足时退到更低档继续产出，`silent` 则跳过本次动作、宁可不出现也不出 OOC。账号驱动模型详见 `docs/m5-account-model.md`。

氛围账号的职责是让数字活起来（点赞数 / 转发数 / 回复区人气），内容质量要求低。核心账号（纯 LLM / 半 LLM）的人设档案含性格、立场、文风、兴趣标签、活跃时段、行为概率参数、情绪状态；核心账号中必须设计性地包含杠精、乐子人、立场偏激者——负面角色是内容资产，不是事故。
...
**内容池管理面板**：编辑内容池的 ECS 三层（组件类型 / 语法 / 池，含维度与准用门槛），新建 / 修改 / 删除条目，并经模拟器的活引擎实时预览组装结果；保存写世界文件夹文件、模拟器热重载即生效。话题评论池的水位与补水属 GM 导演层（后置）。
