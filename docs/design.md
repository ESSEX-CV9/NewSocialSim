...
| 档位 | 数量 | 驱动 | LLM 成本 |
|---|---|---|---|
| 核心账号（组织账号如媒体号 + 个人 NPC） | 10–20 | LLM 按人设生成 | 高 |
| 氛围账号 | 大量、可一次性 | 规则 / 模板引擎 + 内容池 | 池化均摊 |
| 真人用户 | 不限 | 真人 | — |

氛围账号的职责是让数字活起来（点赞数 / 转发数 / 回复区人气），内容质量要求低。核心账号的人设档案含性格、立场、文风、兴趣标签、活跃时段、行为概率参数、情绪状态；核心账号中必须设计性地包含杠精、乐子人、立场偏激者——负面角色是内容资产，不是事故。

simulator 工作区将采用 ECS 架构：Entity = 虚拟账号（网站侧 `is_bot` 用户），Component = 人设档案 / 活跃时段 / 行为概率 / 情绪状态，System = 各驱动源对应的调度系统。模拟器通过 HTTP API 操作网站，不侵入网站代码。

GM 层站在全部账号之上：低频读世界状态摘要（近期热帖文摘、话题热度表、情绪指标），输出导演指令——注入话题、调节话题热度、注入热帖、调整 NPC 情绪参数、安排事件。GM 是上帝控制台的自动驾驶形态：用户手动操作控制台与 GM 指令走同一组管理 API。
...
- ContentGenerator 接口下设 LLMProvider 抽象，优先适配 Claude / DeepSeek / Gemini 三家；第一版 ContentGenerator 为模板 / 语料库实现（行为先跑通），LLM 实现可直接替换。
...
- 提示词缓存纪律（硬约束）：提示词 = [世界索引卡 + 人设卡]（稳定前缀，字节级不变）+ [近期记忆 / 时间线上下文]（缓变）+ [本次触发内容]（每次不同）。索引卡与人设卡必须低频批量更新——改一字即全员缓存失效。三家均有上下文缓存（Claude 显式 cache_control、DeepSeek 自动命中、Gemini 隐式 / 显式），缓存友好是 LLMProvider 接口层的约束，不是各实现自行的优化。
...
1. **世界工作台**（先行）：世界元数据编辑（PATCH，当前世界创建后不可改）、时钟控制 API 与 UI（SimClock 已实现全部方法，缺路由暴露）、世界复制 / 快照 / 删除 API（"复制文件夹 = 平行宇宙"机制的 API 化）、设定文件库（导入 / 索引卡 / 读取 API）、重要账号设计器（创建 NPC 账号并挂人设档案）。完成后世界管理本身即是可用的创作工具，也是模拟器开发的实验台（建测试世界 → 跑 → 暂停回看 → 回滚快照重跑）。
2. **M5 骨架**：虚拟用户管理 API（批量注册、为虚拟用户签发 token 或模拟器管理密钥）→ ECS tick 循环 + 模板版 ContentGenerator 最小闭环（一个 bot 在测试世界定时发帖）→ 议程 / 话题系统与互动级联 → 内容池与计数注水 → LLMProvider 与核心 NPC 双触发 → GM 导演层。
