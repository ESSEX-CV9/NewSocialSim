...
- **Phase 1 顶层帖（内容池 ECS）已完成 1.0–1.5**（详见日志 `2026-06-18.md`）：1.0 TuningService（直读文件）/ 1.1 内容池三层 schema 与加载 / 1.2 组装引擎（混合式取片段 + 占位符 + 种子复现）/ 1.3 shape 过滤 / 1.4 PostingSystem 接组装（按 poolAffinities 选池→组装→发 standalone→轨迹带池·语法·模块）/ 1.5 选池准用门槛（`Pool.tiers` 作者显式勾选哪类账号可用，缺省=谁都不能用；选池两层=tier 门槛筛 + poolAffinities 权重挑；氛围号水贴/核心号场景分流）。**1.1b 话题拆分经勘察重新定性为「不拆不迁移、押后」**（topics 表非用户可见，详见 `docs/m5-x-phase1-baseline.md`「话题表定性」）。**下一步 1.6 内容池面板 / 1.7 NPC 设计器 / 1.8 块详情增强（编辑器面板）→ 1.9 端到端验收。** 账号驱动模型已对齐落档 `docs/m5-account-model.md`（账号类型=默认驱动模式/四轴+降级地板/五预设/纯 LLM 退出确定性互动层），落字段在 1.7。状态机层、LLM 行为层、GM 导演层（M5-5）、Electron 整体打包（M5-6）顺延。
