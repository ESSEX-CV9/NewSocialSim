...
- **Phase 1 顶层帖（内容池 ECS）已完成 1.0–1.6**（详见日志 `2026-06-18.md` + 子日志 `2026-06-18-内容池面板.md`）：1.0 TuningService / 1.1 三层 schema 与加载 / 1.2 组装引擎 / 1.3 shape 过滤 / 1.4 PostingSystem 接组装 / 1.5 选池准用门槛（`Pool.tiers`，两层=tier 门槛 + poolAffinities 权重）/ **1.6 内容池面板**（编辑器侧：模拟器 localhost 控制端点供预览 + 池子文件 fs watch 热重载；编辑器后端三层池读写 + 预览代理；面板=卡片条语法编辑器 + 分组(子文件夹) + 拖拽 + 预览器按槽位可视化；架构见 re-plan「编辑器↔模拟器控制通道」）。**内容池文件改一条一文件、文件名即名字、分组=子文件夹。** **1.1b 话题拆分经勘察重新定性为「不拆不迁移、押后」**（topics 表非用户可见）。**下一步 1.7 NPC 设计器 / 1.8 块详情增强（编辑器面板）→ 1.9 端到端验收。** 账号驱动模型已对齐落档 `docs/m5-account-model.md`（账号类型=默认驱动模式/四轴+降级地板/五预设/纯 LLM 退出确定性互动层），落字段在 1.7（届时内容池"准用门槛"由 tier 切到驱动模式）。状态机层、LLM 行为层、GM 导演层（M5-5）、Electron 整体打包（M5-6）顺延。
- **编辑器跨面板共享态**：跨 dockview 面板的共享数据（时间轴选中态 `selection.ts`、内容池数据 `pools-bus.ts`、预览来源 `preview-bus.ts`）一律走模块级 pub/sub 单例，**不在各面板复制快照**——否则多面板/弹窗数据不同步。
