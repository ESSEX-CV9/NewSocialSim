...
- `server/` — Fastify + better-sqlite3。
  - `src/core/` 基础设施：`clock/`（SimClock 模拟时钟）、`db/`（连接 + 版本化 migration，当前 v3）、`world/`（WorldManager 多世界热切换）、`auth/`（JWT 密钥与 requireAuth/optionalAuth 守卫）、`pagination.ts`（游标工具）。
...
- 未实现的大块：媒体系统（图片上传，影响 banner/媒体 Tab/发帖图片）、自定义历法换算、生产构建流程。
